...
all_sequences = [
    'ACCAD',
    'BioMotionLab_NTroje',
    'BMLhandball', # extra dataset
    'BMLmovi', # extra dataset
    'CMU',
    'CNRS', # extra dataset
    'DanceDB', # extra dataset
        ...
        for action in actions:
            ...
            try:
                ...
                if 'mocap_frame_rate' in data:
                    ...
                    # print(f"\nSkipping {fname}, No mocap framerate present.\n")
                    # print(data.files)
                    continue
                ...
                if pose.shape[0] < 60:
                    print(f"\nSkipping {fname}, less than 60 frames after resampling.\n")
                    continue
                ...
                print(f"\nError loading file {fname}: {e}")
                continue
    if not thetas:
        print(f"\nNo valid data found in {folder}.")
...
def read_seq_data(folder, nsubjects, fps):
    ...
    for subject in subjects[:nsubjects]:
        ...
        for action in actions:
            try:
                np.load(osp.join(folder, subject, action), allow_pickle=True)
            except OSError as e:
